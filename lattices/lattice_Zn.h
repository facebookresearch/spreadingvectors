...
/** returns the nearest vertex in the sphere to a query. Retunrs only
...
struct VectorCodec {
    ...
    VectorCodec(int dim): nv(0), dim(dim) {}
 ...
 * the atom numbers */
struct ZnSphereCodec: ZnSphereSearch, VectorCodec {

    struct CodeSegment:Repeats {
        CodeSegment(const Repeats & r): Repeats(r) {}
 ...
 */
struct ZnSphereCodecRec: VectorCodec {
